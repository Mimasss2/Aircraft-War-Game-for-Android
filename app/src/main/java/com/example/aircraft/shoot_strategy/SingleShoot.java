...
public class SingleShoot implements ShootStrategy {
    ...
    public List<BaseBullet> shoot(int LocationX, int LocationY, int speedY, int direction, int power, boolean is_hero){
        ...
        int speedX = 0;
        if(is_hero) {
            baseBullet = new HeroBullet(LocationX, LocationY + direction*2, speedX, speedY + direction*10, power);
        }
        else {
            baseBullet = new EnemyBullet(LocationX, LocationY + direction*2, speedX, speedY + direction*10, power);
        }
        res.add(baseBullet);
        return res;
    }
